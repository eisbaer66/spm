...
public class DependencyResultTests
{
    ...
    {
        Assert.Throws<ArgumentNullException>(() => new DependencyResult(null!, "verb"));
    }

    [Test]
    public void ConstructorThrowsArgumentNullExceptionIfVerbIsNull()
    {
        Assert.Throws<ArgumentNullException>(() => new DependencyResult(new List<IDependencyLock>(), null!));
    }

    [Test]
    public void ConstructorThrowsArgumentExceptionIfVerbIsEmpty()
    {
        Assert.Throws<ArgumentException>(() => new DependencyResult(new List<IDependencyLock>(), string.Empty));
    }

    [Test]
    public void ConstructorThrowsArgumentExceptionIfVerbIsWhitespace()
    {
        Assert.Throws<ArgumentException>(() => new DependencyResult(new List<IDependencyLock>(), " "));
    }
    ...
    {
        new object[] { Array.Empty<IDependencyLock>(), "no dependencies {Verb}." },
        new object[]
        {
            new IDependencyLock[]
            {
            ...
            "dependency {DependencyId} {Verb} version {Version} to {DownloadPath}.",
        },
        new object[]
        {
            new IDependencyLock[]
            ...
            "{DependencyCount} dependencies {Verb}: {@Dependencies}",
        },
    };
    ...
    {
        var result = new DependencyResult(dependencies, "updated");
