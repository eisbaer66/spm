...
public class GitHubTagZipDependency : GitHubTagDependencyBase, IDependency
{
    ...
    {
        var owner        = Read(dict, "owner");
        var repository   = Read(dict, "repository");
        var assetName    = Read(dict, "assetName");
        var fileInZip    = Read(dict, "fileInZip");
    ...
    private static string Read(IDictionary<string, string> dict, string key)
    {
        if (!dict.ContainsKey(key))
        {
            throw new ArgumentException("dict is missing the property '" + key + "'", nameof(dict));
        }
