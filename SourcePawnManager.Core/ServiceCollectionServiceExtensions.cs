...
public static class ServiceCollectionServiceExtensions
{
                                                          ...
                                                          string?                 githubToken = null)
    {
        ...
        services.AddHttpClient(GitHubApi.ApiHttpClientName,
                               (sp, client) => ConfigureClient(client, sp, "application/json"));
        services.AddHttpClient(GitHubApi.DownloadHttpClientName,
                               (sp, client) => ConfigureClient(client, sp, "application/octet-stream"));
    ...
    private static void ConfigureClient(HttpClient client, IServiceProvider serviceProvider, string acceptHeader)
    {
        client.BaseAddress = new(GitHubBaseAddress);
        client.DefaultRequestHeaders.Accept.ParseAdd(acceptHeader);
        client.DefaultRequestHeaders.UserAgent.Add(ProductInfoHeaderValue.Parse("icebear.SourcePawnManager"));
