...
public class SideFileVersionStore : IVersionStore
{
    ...
    {
        var lookup = dependencies.Select(d => d.DownloadPath + ".version").Select(f => Path.GetFullPath(Path.Combine(Environment.CurrentDirectory, f))).ToHashSet();
        bool NotUsed(string f) => !lookup.Contains(f);
