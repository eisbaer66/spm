...
{
    public UpdateCommand() : base("update", "updates installed includes to the latest version according to spm.json")
    {
        AddAlias("u");
    }
    ...
    public class CommandHandler : CommandHandlerBase
    {
        ...
        protected override async Task<IResult> InvokeAsync(CancellationToken cancellationToken) =>
            await _includeManager.Update(Environment.CurrentDirectory, cancellationToken);
    }
}